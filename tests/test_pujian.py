...
def test_folder_english_way_prefers_exact_song_folder(drive_folder_payload):
    from Pujian import folder_english_way
...
def test_make_local_folder_based_on_google_folder_name_creates_song_folder(
    ...
    from Pujian import make_local_folder_based_on_google_folder_name
...
def test_download_new_song_pipeline_uses_selected_drive_folder(
    ...
    monkeypatch.setattr(Pujian, "st_print", lambda text: calls.append(("print", text)))
    monkeypatch.setattr(
        Pujian,
        "get_list_folders",
    ...
    assert Pujian.download_new_song_pipeline("161") == images
