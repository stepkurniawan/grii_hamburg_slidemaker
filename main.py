...
def add_static_page(prs, layout_name):
...
def add_beginning_slide(prs):
    ...
    add_static_page(prs, layout_name)
...
def add_bibel_reading_page(prs):
    # Find the layout with the specified name
    slide_layout = prs.slide_master.slide_layouts[4]

    slide = prs.slides.add_slide(slide_layout)
    try: 
        bible_verse = slide.placeholders[10]
        bible_verse.text = "Jesaya 55:6-11"
    except IndexError:
